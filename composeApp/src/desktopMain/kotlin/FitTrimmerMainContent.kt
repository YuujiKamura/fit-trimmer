...
fun FitTrimmerMainContent(
    ...
    var isInitialLoadOfRestoredVideo by remember { mutableStateOf(!initialCache?.videoPath.isNullOrEmpty()) }
                        ...
                        withContext(Dispatchers.Main) {
                            if (videoPath == originalPathAtStart) {
                                ...
                                if (telemetryPoints.isNotEmpty()) {
                                    val isRestored = isInitialLoadOfRestoredVideo && videoPath == initialCache?.videoPath
                                    if (isRestored) {
                    ...
                    val runBlock = {
                        try {
                            when (cmd) {
                                ...
                                is CpCommand.Fire -> {
                                    scope.launch {
                                        // Unify single encode into the batch queue execution path
                                ...
                                CpCommand.ShowBatchConfirm -> {
                                 ...
                                 CpCommand.EarlyFinish -> {
                                     ...
                                     System.setProperty("fit-trimmer.is-early-finish", "true")
                ...
                Box(modifier = Modifier.width(sidebarWidth).fillMaxHeight()) {
                    ...
                    Card(
                        ...
                        Column(
                            ...
                            ControlSlider("MAP LEVEL OFFSET", settings.mapZoomOffset.toFloat(), -30f, 3f, enabled = !isEncoding) { settings = settings.copy(mapZoomOffset = it.roundToInt()) }
                            ...
                            Spacer(Modifier.height(4.dp))
                            Text("MAP POSITION", color = Color(0xFF1C1C1E), fontWeight = FontWeight.Bold, fontSize = 10.sp, letterSpacing = 0.5.sp)
                            var mapPosDropdownExpanded by remember { mutableStateOf(false) }
                            Box(modifier = Modifier.fillMaxWidth()) {
                                val currentPosLabel = when (settings.mapPosition) {
                                    "bottom_right" -> "Bottom Right (右下)"
                                    else -> "Top Right (右上)"
                                }
                                OutlinedButton(
                                    onClick = { if (!isEncoding) mapPosDropdownExpanded = true },
                                    modifier = Modifier.fillMaxWidth().height(36.dp),
                                    border = BorderStroke(1.dp, Color(0xFFE5E5EA)),
                                    colors = ButtonDefaults.outlinedButtonColors(contentColor = Color(0xFF1C1C1E))
                                ) {
                                    Row(
                                        ...
                                        Text(currentPosLabel, fontSize = 11.sp)
                                        Text("▼", fontSize = 8.sp, color = Color.Gray)
                                    }
                                }
                                DropdownMenu(
                                    expanded = mapPosDropdownExpanded,
                                    onDismissRequest = { mapPosDropdownExpanded = false }
                                ) {
                                    DropdownMenuItem(onClick = { settings = settings.copy(mapPosition = "top_right"); mapPosDropdownExpanded = false }) {
                                        Text("Top Right (右上)", fontSize = 11.sp, color = Color(0xFF1C1C1E))
                                    }
                    ...
                    Column(
                        ...
                        if (!isActiveEncoding) {
                            VideoPreviewArea(
                                ...
                                videoCurrentTimeMsProvider = { videoCurrentTimeMs },
                        ...
                        Card(
                            ...
                            Column(
                                ...
                                Row(
                                    ...
                                    Button(
                                        onClick = {
                                            viewModel.restorePendingBatchJobs(scope)
                                        },
