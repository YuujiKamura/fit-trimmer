 ...
 */
private object windowsLogger {
    fun e(msg: () -> String) {
        System.err.println("ERROR [WindowsVideoPlayerState]: ${msg()}")
    }
    fun w(msg: () -> String) {
        System.err.println("WARN [WindowsVideoPlayerState]: ${msg()}")
    }
    fun i(msg: () -> String) {
        println("INFO [WindowsVideoPlayerState]: ${msg()}")
    }
}
...
class WindowsVideoPlayerState : PlatformVideoPlayerState {
    ...
    internal val bitmapLock = ReentrantReadWriteLock()
    internal val currentFrameState = mutableStateOf<ImageBitmap?>(null)
    internal var frameTicks by mutableStateOf(0L)
     ...
     * We monitor [_isPlaying] and block indefinitely until it becomes true.
     * The original 5-second auto-restart timeout (watchdog) has been removed here
     * because it incorrectly triggered during manual pauses.
     */
